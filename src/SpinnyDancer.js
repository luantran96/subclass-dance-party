var SpinnyDancer = function(top, left, timeBetweenSteps) {
  ...
  this.$node.on('click', () => {
    ...
    for( var i = 0; i < window.dancers.length; i++){
        ...
        if(currentDancer.$node.hasClass('jimmy') && !currentDancer.danceWithTina){
      
            var jimmyLeft = currentDancer.left;
            var jimmyTop = currentDancer.top;
          
            this.$node.animate({left: jimmyLeft + 100, top: jimmyTop},2000);
            this.danceWithJimmy = true;
            currentDancer.danceWithTina = true;
